...
export const defineCanvas = () => {
	...
	// if(/https?:\/\/pixelzone\.io.*/.test(location.origin)) {
	// 	return CANVASES.PZ;
	// }
}
...
export const numberToVec2 = (() => {
	const view = new DataView(new ArrayBuffer(4));
	return (n: number) => {
		view.setInt32(0, n);
		return <Vec2>[view.getInt16(0), view.getInt16(2)];
	}
})();
...
export const createHeatmap = (/*api: CanvasAPI,*/ targeter: ITargeter) => {
	...
	const { width, height, targets } = targeter;
