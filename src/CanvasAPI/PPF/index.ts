...
class API extends SuperAPI implements ILowLevelAPI {
	...
	readonly pixelsDelay = 150 + Math.random() * 50
	private delayedPixels: Array<Array<Pixel>> = []
		...
		this.ws.on(WS_EVENTS.BINARY, data => {
			...
			switch(view.getUint8(0)) {
				case PIXEL_UPDATE_OP:
					this.handleArrivedPixel(hydratePixelUpdate(view));
					break;
	...
	public getHumanlike(x: number, y: number) {
	...
	private handleArrivedPixel(packet: PixelUpdatePacket) {
