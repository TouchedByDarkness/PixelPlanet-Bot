...
if(!basicTargeters[DEFAULT_STRATEGY]) {
...
const targeter = new AsyncSingleton(async () => {
	...
	const strategy = (
		global.storage.has('strategy') && <string>global.storage.get('strategy') in basicTargeters ?
		<string>global.storage.get('strategy') :
		DEFAULT_STRATEGY);

	const start = performance.now();
	const targeter = createBasicTargeter(_api, _template, strategy) || null;
	if(!targeter) {
...
(async () => {
	...
	Object.keys(basicTargeters).forEach(strategy => gui.addStrategy(strategy));
	...
	bus.on(APP_EVENTS.SHOW_HEATWEB, async () => {
		const _targeter = await targeter.get();
		if(!_targeter) {
			return;
		}
