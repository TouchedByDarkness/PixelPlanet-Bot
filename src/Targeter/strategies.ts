...
export default <Record<string, SortingFunc>>{
	...
	woyken: (tmp: Template) => {
		...
		const gridSize = 8;
		const [centerX, centerY] = tmp.localCenter;
		return sortAscending(createTargets(tmp), (t: Target): number => {
			const x = t[0]
			const y = t[1]
	...
	zipper: (tmp: Template) => sortDescending(createTargets(tmp), t => Math.trunc(Math.sqrt(t[0] + t[1]))),
	zipper2: (tmp: Template) => sortDescending(createTargets(tmp), (t: Target): number => 
	...
	alienRandom: (tmp: Template) => createTargets(tmp).sort((a, b) => Math.tan(sq(a[0] - b[1]) - sq(a[1] - b[0]))),
	alien1: (tmp: Template) => sortDescending(createTargets(tmp), t => Math.tan(sq(t[0]) + sq(t[1]))),
	alien2: (tmp: Template) => {
	...
	complex: (tmp: Template) => {
		let [borders, internals] = segmentize(tmp, createTargets(tmp));

		const distance = (t1: Vec2, t2: Vec2): number => sq(t1[0] - t2[0]) + sq(t1[1] - t2[1]);
		...
		const borderCells = sortAscending(
			...
			(border: Array<Target>): number => border.length);
		borders = internals = [];

		const internalCenters = internalCells.map(targetsCenter);
		const usedInternals: Array<number> = [];

		const parts: Array<Array<Target>> = [];
		for(let i = 0; i !== borderCells.length; i++) {
			...
			internalCells.forEach((internal, j) => {
				...
				const d = distance(borderCenter, internalCenters[j]);
		...
		return ([] as Array<Target>).concat(...parts);
	},
	binary: (tmp: Template) => sortDescending(createTargets(tmp), t => t[0] & t[1]),
	near: (tmp: Template) => {
		return sortNear(shuffle(createTargets(tmp)));
		// const all = shuffle(createTargets(tmp));
...
function sortNear(targets: Array<Target>): Array<Target> {
	if(targets.length < 5) {
